...
/**
 * \file gtkInfoItem.c
 * \brief connectionMgr application - setup item showing additional help or info text`
 * 
 */
...
GType gtk_infoitem_get_type(void)
{
    ...
    if (!infoitem_type)
    {
        ...
        infoitem_type = g_type_register_static(GTK_TYPE_EVENT_BOX, "InfoItem", &infoitem_info, 0);
    }
...
static void gtk_infoitem_init(GtkInfoItem * item)
{
    ...
    gtk_widget_set_size_request(GTK_WIDGET(item), INFOITEM_REQ_WIDTH, INFOITEM_REQ_HEIGHT);
    ...
    gtk_widget_set_size_request(widget, INFOITEM_ICON_IMG_WIDTH, INFOITEM_ICON_IMG_HEIGHT);
    ...
    gtk_widget_set_size_request(widget, INFOITEM_TEXT_WIDTH, INFOITEM_TEXT_HEIGHT);
    ...
    gtk_widget_set_size_request(widget, INFOITEM_BUTTON_WIDTH, INFOITEM_BUTTON_HEIGHT);
    gtk_container_add(GTK_CONTAINER(alignment), widget);
    gtk_widget_show(widget);
    item->button = widget;
    //     signal callbacks
    g_signal_connect_after(G_OBJECT(widget), "toggled", G_CALLBACK(on_button_toggled), item);
